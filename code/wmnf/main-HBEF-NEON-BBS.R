...
source("code/wmnf/nimble-code/icom-HBEF-NEON-BBS.R")
...
elev.bbs <- elev[1:J.bbs + J.hbef + J.neon]
for.hbef <- percent.for[1:J.hbef]
for.neon <- percent.for[1:J.neon + J.hbef]
for.bbs <- percent.for[1:J.bbs + J.hbef + J.neon]
years.neon <- 6:9
...
icom.consts <- list(n.years = n.years, n.years.neon = n.years.neon, I = I, 
		   ...
		   year.indx.neon.all = v.1.df$Year.all, n.obsv.bbs = n.obsv.bbs, 
                   ...
                   sp.indx.bbs.p = v.2.df$p.sp, 
		   J.hbef = J.hbef, J.neon = J.neon, J.bbs = J.bbs, 
		   years.neon = years.neon, sp.neon = sp.neon, 
...
save(icom.data, icom.consts, v.2.df, y.df, v.1.df, elev.real, percent.for.real,
     file = 'data/nimble-data.R')
# Initial values --------------------------------------------------------
...
file.name <- paste('results/icom-HBEF-NEON-BBS-results-', n.iter, '-iterations-', chain, '-chain-', 
		   date, '.R', sep = '')
