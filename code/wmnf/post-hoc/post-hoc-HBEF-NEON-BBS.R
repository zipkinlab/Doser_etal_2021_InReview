# post-hoc-NEON-BBS.R: code to summarize NIMBLE results from model using HBEF, 
...
# Read in results ---------------------------------------------------------
load("results/icom-HBEF-NEON-BBS-results-450000-iterations-1-chain-2021-08-29.R")
samples.a <- samples
load("results/icom-HBEF-NEON-BBS-results-450000-iterations-2-chain-2021-08-29.R")
samples.b <- samples
load("results/icom-HBEF-NEON-BBS-results-450000-iterations-3-chain-2021-08-29.R")
samples.c <- samples
...
samples <- samples.list[, -c(z.hbef.indx, z.bbs.indx, z.neon.indx, 
			     psi.hbef.indx, psi.bbs.indx, psi.neon.indx)]
...
psi.neon.samples <- mcmc.list(mcmc(samples.a[, psi.neon.indx]))
...
# Compute Jaccard index and growth rate for each site
...
z.neon.wide <- array(0, dim = c(n.iter, I.hbef, J.neon, T.hbef))
neon.sp.indx <- unique(v.1.df$Species)
neon.year.indx <- unique(v.1.df$Year.all)
z.neon.wide[, neon.sp.indx, , neon.year.indx] <- array(z.neon.samples,
					                 dim = c(n.iter, I.neon, J.neon, T.neon))
# Species richness
rich.neon.samples <- apply(z.neon.wide, c(1, 3, 4), sum)
rich.neon.samples <- rich.neon.samples[, , neon.year.indx]
# Compute Jaccard index
jaccard.neon.samples <- array(NA, dim = c(n.iter, J.neon, T.neon))
ref.site <- 1
for (a in 1:n.iter) {
  print(paste('Currently on iteration ', a, ' out of ', n.iter, sep = ''))
  for (j in 1:J.neon) {
    for (t in 1:T.neon) {
      jaccard.neon.samples[a, j, t] <- sum(z.hbef.wide[a, , ref.site, neon.year.indx[t]] * z.neon.wide[a, , j, neon.year.indx[t]]) / 
	      (sum(z.hbef.wide[a, , ref.site, neon.year.indx[t]]) + sum(z.neon.wide[a, , j, neon.year.indx[t]]) - 
	      sum(z.hbef.wide[a, , ref.site, neon.year.indx[t]] * z.neon.wide[a, , j, neon.year.indx[t]]))
    } # t
  } # j
} # a 
...
psi.neon.wide[, neon.sp.indx, , neon.year.indx] <- array(psi.neon.samples,
					                 dim = c(n.iter, I.neon, J.neon, T.neon))
psi.all.wide <- abind(psi.hbef.wide, psi.neon.wide, psi.bbs.wide, along = 3)
...
save(samples, jaccard.bbs.samples, rich.bbs.samples,
     jaccard.neon.samples, rich.neon.samples, jaccard.hbef.samples, rich.hbef.samples,
     psi.avg.sp.year, beta.samples, sigma.sq.samples, 
     file = 'results/icom-HBEF-NEON-BBS-results.R')
