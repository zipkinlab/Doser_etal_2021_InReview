# post-hoc-NEON-BBS.R: code to summarize NIMBLE results from model using 
...
# Read in results ---------------------------------------------------------
load("results/icom-NEON-BBS-results-450000-iterations-1-chain-2021-08-19.R")
samples.a <- samples
load("results/icom-NEON-BBS-results-450000-iterations-2-chain-2021-08-19.R")
samples.b <- samples
load("results/icom-NEON-BBS-results-450000-iterations-3-chain-2021-08-19.R")
samples.c <- samples
...
samples <- samples.list[, -c(z.bbs.indx, z.neon.indx, 
			     psi.bbs.indx, psi.neon.indx)]
...
for (a in 1:n.iter) {
  ...
  for (j in 1:J.bbs) {
    for (t in 1:T.bbs) {
      jaccard.bbs.samples[a, j, t] <- sum(z.bbs.wide[a, , ref.site, t] * z.bbs.wide[a, , j, t], na.rm = TRUE) / 
	      (sum(z.bbs.wide[a, , ref.site, t], na.rm = TRUE) + sum(z.bbs.wide[a, , j, t], na.rm = TRUE) - 
...
# Make this the same size as the DET data, just fill in the correct years
...
# Post-hoc linear regression as trend estimate --------------------------
...
psi.neon.wide <- array(NA, dim = c(n.iter, I.bbs, J.neon, T.bbs))
psi.neon.wide[, neon.sp.indx, , neon.year.indx] <- array(psi.neon.samples, 
					                 dim = c(n.iter, I.neon, J.neon, T.neon))
psi.all.wide <- abind(psi.bbs.wide, psi.neon.wide, along = 3)
...
save(samples, jaccard.neon.samples, rich.neon.samples, 
     jaccard.bbs.samples, rich.bbs.samples, psi.avg.sp.year, 
     beta.samples, sigma.sq.samples,
     file = 'results/icom-NEON-BBS-results.R')
