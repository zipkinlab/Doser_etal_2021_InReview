# idm-HBEF-NEON-BBS.R: BUGS code to run IDM in NIMBLE using HBEF, NEON, and BBS
...
idm.code <- nimbleCode({
  ...
  gamma.1.1 ~ dnorm(0, 0.368) # day
  gamma.1.2 ~ dnorm(0, 0.368) # day^2
  gamma.1.3 ~ dnorm(0, 0.368) # hour
  ...
  for (t in 1:n.years.neon) {
    gamma.1.0[t] ~ dnorm(gamma.1.0.mean, tau.gamma.1.0)
  }
  ...
  # NEON ------------------------------
  for (j in 1:J.neon) {
    logit(psi.neon[j, 1]) <- beta.0[years.neon[1]] + 
                            beta.1 * ELEV.neon[j] + 
        	            beta.2 * pow(ELEV.neon[j], 2) + 
        		    beta.3 * FOREST.neon[j]
    z.neon[j, 1] ~ dbern(psi.neon[j, 1])
    for (t in 2:n.years.neon) {
    logit(psi.neon[j, t]) <- beta.0[years.neon[t]] + 
                            beta.1 * ELEV.neon[j] + 
      		            beta.2 * pow(ELEV.neon[j], 2) + 
        		    beta.3 * FOREST.neon[j] + 
        		    phi * z.neon[j, t - 1]
      z.neon[j, t] ~ dbern(psi.neon[j, t])
    } # t
  } # j
  # BBS -------------------------------
  for (j in 1:J.bbs) {
  ...
  for (i in 1:n.vals.neon) {
    logit(pi.1[i]) <- gamma.1.0[year.indx.neon[i]] + 
                      gamma.1.1 * DAY.neon[i] + 
        	      gamma.1.2 * pow(DAY.neon[i], 2) + 
        	      gamma.1.3 * HOUR.neon[i]
    v.1[i] ~ dbern(pi.1[i] * z.neon[site.neon[i], year.indx.neon[i]])
