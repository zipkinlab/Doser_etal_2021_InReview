...
# Simulated number of sites with HB and NEON data
R.hb.vals <- c(5, 20, 50, 100)
R.neon.vals <- R.hb.vals
...
for (i in 1:n.sims) {
  ...
  if (i %in% c(5, 10)) {
    date <- Sys.Date()
    file.name <- paste('results/sim-icm-results-1-', i, '-simulations-', date, '.R', sep = '')
    save(out.model, beta.psi.0, beta.psi.1, beta.phi.0,
	 beta.phi.1, beta.gamma.0, beta.gamma.1, R.hb.vals, file = file.name)
  }
}
