# main-sim-2-icm.R: this script assess the integrated community model 
#                   performance for a fixed community of 9 bird species. 
#                   These species are highly variable in their 
#                   characteristics. Further, estimates are compared 
#                   across models using different amounts of the four 
#                   data types. 
rm(list = ls())
library(nimble)
library(coda)
...
# Run subsequent simulations using the same pixel values
for (j in 2:n.sims) {
  set.seed(my.seeds[j])
...
file.name <- paste('results/sim-icm-results-2-', n.sims, '-condition-', index, '-simulations-', date, '.R', sep = '')
