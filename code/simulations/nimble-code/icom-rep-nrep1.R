# icom-rep-nrep1.R: BUGS code for running ICOM with one replicated
...
# Constants -----------------------------------------------------------
icom.consts <- list(I = I, J = J.rep + J.nrep.1, n.years = n.years, K.rep = K.rep,
    	     J.rep = J.rep, K.nrep.1 = K.nrep.1, J.nrep.1 = J.nrep.1,
    	     site.rep = 1:J.rep, site.nrep.1 = 1:J.nrep.1 + J.rep)

# Data --------------------------------------------------------------------
psi.indices <- 1:(J.rep + J.nrep.1)
icom.data <- list(y = dat$y, v.1 = dat$v.1, 
    	         X.psi = dat$X.psi[psi.indices, ], X.rep = dat$X.rep,
		 X.nrep.1 = dat$X.nrep.1)

# Initial values ----------------------------------------------------------
icom.inits <- list(z = array(1, dim = c(I, J.rep + J.nrep.1, n.years)),
