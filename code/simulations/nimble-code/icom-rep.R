# icom-rep.R: BUGS code, initial values, constants, and data used to run 
#             community model in NIMBLE using a single replicated data source
    ...
    for (t in 1:n.years) {
      ...
      for (j in 1:J) {
        for (k in 1:K.rep) {
          ...
          y[i, j, k, t] ~ dbern(p[i, j, k, t] * z[i, j, t])
        } # k 
      } # j 
    } # t
  } # i
...
# Constants -----------------------------------------------------------
icom.consts <- list(I = I, J = J.rep, n.years = n.years, K.rep = K.rep)

# Data --------------------------------------------------------------------
psi.indices <- c(1:J.rep)
icom.data <- list(y = dat$y, X.psi = dat$X.psi[psi.indices, ], X.rep = dat$X.rep)

# Initial values ----------------------------------------------------------
icom.inits <- list(z = array(1, dim = c(I, icom.consts$J, n.years)),
