# icom-rep-nrep2.R: BUGS code for running ICOM with one replicated
#                   data source and one nonreplicated data source.
...
icom.code <- nimbleCode ({
  ...
  beta.1.mean ~ dnorm(0, 0.368)
  phi.mean ~ dnorm(0, 0.368) # auto-logistic parameter
  # Replicated data
  alpha.1.mean ~ dnorm(0, 0.368)
  # Nonreplicated data 2
  gamma.2.1.mean ~ dnorm(0, 0.368)
...
# Constants -----------------------------------------------------------
icom.consts <- list(I = I, J = J.rep + J.nrep.2, n.years = n.years, K.rep = K.rep,
    	     J.rep = J.rep, 
    	     site.rep = 1:J.rep, 
    	     site.nrep.2 = 1:J.nrep.2 + J.rep, J.nrep.2 = J.nrep.2)

# Data --------------------------------------------------------------------
psi.indices <- c(1:J.rep, 1:J.nrep.2 + J.nrep.1 + J.rep)
icom.data <- list(y = dat$y, v.2 = dat$v.2,
    	         X.psi = dat$X.psi[psi.indices, ], 
		 X.rep = dat$X.rep, X.nrep.2 = dat$X.nrep.2)

# Initial values ----------------------------------------------------------
icom.inits <- list(z = array(1, dim = c(I, J.rep + J.nrep.2, n.years)),
