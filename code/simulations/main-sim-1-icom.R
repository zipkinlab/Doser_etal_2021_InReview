...
#    high detection
# For example: to run the model using all three data sources, run the following
#              on the command line from the home directory for this project
#    Rscript code/simulations/main-sim-1-icom.R 7 & 
index <- as.numeric(commandArgs(trailingOnly = TRUE))
if(length(index) == 0) base::stop('Need to tell NIMBLE which model to run')
...
curr.model <- paste('code/simulations/nimble-code/', code.files[index], sep = '')
...
file.name <- paste('results/sim-icom-results-1-', n.sims, '-condition-', index, '-simulations-', date, '.R', sep = '')
