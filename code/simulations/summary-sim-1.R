...
for (i in 1:n.scenarios) {
  ...
  curr.mean <- sapply(samples.list, FUN = function(a) {apply(a, 2, median)})
...
plotting.dat <- plotting.dat %>%
  mutate(med = ifelse(med > 2, NA, med), 
	 low = ifelse(med > 2, NA, low), 
	 high = ifelse(med > 2, NA, high))
...
ggarrange(int.sp.plot, cov.sp.plot, labels = c('A', 'B'), 
  ...
  ggsave(device = 'pdf', filename = 'figures/sim-1-sp-int-cov.pdf', 
...
ggarrange(int.comm.plot, cov.comm.plot, labels = c('A', 'B'), 
	  font.label = list(size = 18)) 
ggsave(device = 'pdf', filename = 'figures/sim-1-comm-int-cov.pdf', 
       width = 12, height = 7, units = 'in')
