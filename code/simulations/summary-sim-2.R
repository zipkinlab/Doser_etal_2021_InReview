...
#       can be generated using the main-sim-2-icom.R file, or contact
#       the first author for the files (doserjef@msu.edu) for a link to 
#       a Dropbox folder with the results.  
load('results/sim-icom-results-2-100-simulations-1-chain-2021-08-26.R')
samples.a <- samples.list
load('results/sim-icom-results-2-100-simulations-2-chain-2021-08-26.R')
samples.b <- samples.list
load('results/sim-icom-results-2-100-simulations-3-chain-2021-08-26.R')
...
# Results (each of the loaded files) consist of the following objects: 
...
comm.mean <- sapply(samples.list, FUN = function(a) {apply(do.call('rbind', a), 2, mean)})
# Compute 2.5% quantile for Bayesian CI
comm.bci.low <- sapply(samples.list, FUN = function(a) {apply(do.call('rbind', a), 2, quantile, 0.025)})
# Compute 97.5% quantile for Bayesian CI
comm.bci.high <- sapply(samples.list, FUN = function(a) {apply(do.call('rbind', a), 2, quantile, 0.975)})
...
for (i in 1:n.sp) {
  ...
  #     gamma.1.1: true nonreplicated data set detection covariate effect
  # Compute mean for each parameter and simulation
  curr.mean <- sapply(samples.list, FUN = function(a) {apply(do.call('rbind', a), 2, mean)})
...
comm.low$params <- rep(c('alpha.0.1', 'alpha.0.2', 'alpha.0.3', 'alpha.0.4', 
		       ...
		       'beta.0.5', 'beta.0.6', 'beta.1',
		       'gamma.1.0.1', 'gamma.1.0.2', 'gamma.1.0.3', 'gamma.1.0.4', 
		       'gamma.1.0.5', 'gamma.1.0.6', 'gamma.1.1', 'phi'), each = n.sp)
...
# Figure S2
ggsave(device = 'pdf', filename = 'figures/FigS2.pdf', 
       height = 5, width = 10, units = 'in')

# Compare all parameters and summarize in table ---------------------------
full.df <- full.df %>%
  arrange(params, species)	
# Add true values and a variable used to average across parameters 
# that vary by year. 
full.df <- full.df %>%
...
true <- c(alpha.0[, 1], alpha.0[, 2], alpha.0[, 3],
		  ...
		  beta.1, gamma.1.0[, 1], gamma.1.0[, 2], gamma.1.0[, 3],
		  gamma.1.0[, 4], gamma.1.0[, 5], gamma.1.0[, 6],
		  gamma.1.1, phi)
